...
export async function getChildBlocksContents(
    blockId: BlockId,
): Promise<IResBlock[]> {
    ...
    if (blockIds.length > 0) {
        ...
        ids = ids + ")";
        const blocks: IResBlock[] = await sql(
...
export async function fullTextSearchBlock(
    ...
    //let searchConf = config.conf.search;
    let searchConf = {};
    const confPrefix = [
        "blockquote",
        "codeBlock",
    ...
    for (const [key, val] of Object.entries(config.conf.search)) {
        if (confPrefix.includes(key)) {
            searchConf[key] = val;
        }
    }
